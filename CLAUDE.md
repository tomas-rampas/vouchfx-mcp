...
- The one exception, `RealPlanCoverageAgainstPinnedCliTests`, self-gates by running the production `CliPinVerifier` against the real PATH and silently passes when the pinned CLI is not installed — do not invent a new skip mechanism for CLI-dependent tests; reuse that pattern.
...
- Development is spec-first: features land against approved specs in a spec → build → review loop. `specs/` is gitignored (maintainer-local, mirrors the engine repo's convention). `docs/vouchfx-ai-mcp-spec.md` and `docs/vouchfx-ai-mcp-plan.md` are the current fleet-level spec/plan material; requirement IDs (REQ-NNN/EDGE-NNN) appear throughout code comments and test names.
- `docs/*.md` (overview, install, tools-and-resources, troubleshooting) are the source for the published site at vouchfx-mcp.vouchfx.io; the README's "Status" block is kept current with tool-level detail.
