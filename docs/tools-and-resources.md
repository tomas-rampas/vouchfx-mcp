...
- **Result shape**: `{ normalizedYaml: string | null, commentsDropped: boolean, normalizationRefused: string | null, validation: { valid, errors, semanticDiagnostics, semanticDiagnosticsTruncated, summary, level }, meta }`.
  - `commentsDropped` states a fact about **your document**, not about the pipeline: it is `true` only when the suite you supplied contained at least one real YAML comment **and** canonical text was produced for it. A comment-free suite normalizes with `commentsDropped: false` — it was normalized, and nothing was lost. It is also `false` whenever `normalizedYaml` is `null`, because nothing was produced. Read it as "did my document lose comments", not "did normalization run".
    - The input is measured with the YAML **tokenizer**, not a text search for `#`. A `#` inside a quoted scalar (`name: "a # b"`), in an unspaced plain scalar (`colour: #ffaa00`), in a URL fragment (`https://x.test/p#frag`), or inside a block scalar is not a comment — none of those set the flag, and all of them survive normalization intact.
    - This was a per-document fix ([issue #85](https://github.com/tomas-rampas/vouchfx-mcp/issues/85)). The flag previously reported `true` on every response carrying canonical text, on the reasoning that producing the text and dropping comments are the same act on the pinned library — true of the act, but wrong about a suite that had no comments to begin with.
  ...
  - The reasons `normalizedYaml` can be `null` are told apart by `normalizationRefused` and `validation` — `commentsDropped` is `false` in all of them, so it is not the discriminator. A non-`null` `normalizationRefused` means the emission was refused and names which half of the gate failed. A `null` one means either that normalization was not requested or that there was no document to canonicalise; **`validation` separates those two only when the suite is otherwise fine**, since a suite that is invalid *and* was never asked to be normalized presents exactly as one that could not be canonicalised. If you need that distinction, you already have it: you know whether you passed `normalize: true`.
...
6. **Normalize, then write the file yourself** — `normalize_suite` with **`normalize: true`** (the
   ...
   `normalizationRefused` reason), or the result comes back with `commentsDropped: true` and those
   comments are worth keeping, the host writes the validated YAML it already has instead.
