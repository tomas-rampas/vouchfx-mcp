...
- A pathological but not deliberately malicious YAML input that is slow to parse (e.g. very long lines, or deeply nested flow-style collections that stay under the hard size/depth caps).
...
- If it recurs consistently for the same suite, check whether that suite triggers slow YAML parsing even though it stays under the hard size/nesting/alias caps.
