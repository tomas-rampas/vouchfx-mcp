...
## Explanation
`get_step_timeline` read the run's event stream and found no step with the id you asked about — no
`step-started`, `step-attempt` or `step-completed` event names it.
...
your suite. The same holds for a step that started and never finished, because the run was cut short
mid-step: its `step-started` event is evidence that it exists, so it too gets a successful result:
whatever attempts it recorded before the cut (often none), its declared `timeoutMs` and
`declaredVerifyMode`, and a `conclusion` that says the stream ends without a verdict for it.
