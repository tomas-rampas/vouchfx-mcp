...
mid-step: its `step-started` event is evidence that it exists, so it too gets a successful result, with
an empty `attempts` array, its declared `timeoutMs` and `declaredVerifyMode`, and a `conclusion` that
says the stream ends without a verdict for it.
