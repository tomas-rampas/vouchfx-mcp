...
public class RealSecretHygieneMcpTests
{
    ...
    /// assertion): a twelfth tool added anywhere fails that assertion until it is either swept for
    ...
    /// completes: eight of this server's ELEVEN tools' own structured results, the resources listing,
    /// each resource's read content, the resource TEMPLATES listing, and one concrete instantiation
    /// of the errors template — see the sweep below's own remarks for exactly which eight, and for
    ...
    /// fail-closed set-equality assertion against the live <c>tools/list</c> means a TWELFTH tool,
    ...
    public async Task AcrossTheWholeToolSurface_ServerEnvironmentSentinel_NeverAppearsInAnyResponseOrNotification()
    {
        ...
        try
        {
            ...
            // Eight of this server's ELEVEN advertised tools, driven through the REAL MCP round trip —
            ...
            // assertion against the live tools/list, so a TWELFTH tool (or a shrinking/growing gap)
            ...
            foreach (var result in new[] { validate, listTypes, describe, search, run, explain, explainDiagnostic, getSchema })
            {
            ...
            // tools/list — never merely "at least the seven/eight this test happens to call". A
            // twelfth tool added anywhere fails here immediately, until it is either swept above or
            ...
            var sweptTools = new[]
            {
                ...
                "run_suite", "explain_run", "explain_diagnostic", "get_schema",
            };
            Assert.Equal(
                advertisedTools,
