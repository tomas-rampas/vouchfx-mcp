...
public class RealSecretHygieneMcpTests
{
    ...
    /// assertion): a seventeenth tool added anywhere fails that assertion until it is either swept for
    ...
    /// completes: thirteen of this server's SIXTEEN tools' own structured results, the resources listing,
    /// each resource's read content, the resource TEMPLATES listing, and one concrete instantiation
    /// of the errors template — see the sweep below's own remarks for exactly which thirteen, and for
    ...
    /// fail-closed set-equality assertion against the live <c>tools/list</c> means a SEVENTEENTH tool,
    ...
    public async Task AcrossTheWholeToolSurface_ServerEnvironmentSentinel_NeverAppearsInAnyResponseOrNotification()
    {
        ...
        try
        {
            ...
            // Thirteen of this server's SIXTEEN advertised tools, driven through the REAL MCP round trip —
            ...
            // assertion against the live tools/list, so a SEVENTEENTH tool (or a shrinking/growing gap)
            ...
            foreach (var result in new[]
                     {
                         ...
                         runEvents, runStatus, listRuns, cancelRun,
                     })
            {
            ...
            // seventeenth tool added anywhere fails here immediately, until it is either swept above
            ...
            var sweptTools = new[]
            {
                ...
                "get_run_events", "get_run_status", "cancel_run", "list_runs",
            };
            Assert.Equal(
                advertisedTools,
