...
/// </para>
/// <para>
/// <b>This is the ONE pathway all eighteen tools use to reach the wire</b> — which is why US-S1-02's
/// <c>meta</c> stamp is attached here (see <see cref="Success(object)"/>) instead of as a field on
/// each of the eighteen payload records: a per-payload field is eighteen places that can drift and a
/// future nineteenth tool that can forget, whereas a choke point cannot be bypassed without deleting the
...
internal static class StructuredToolResult
{
    ...
    /// property of any of the eighteen payloads sees it at exactly the path it was at before, since
    ...
    /// loudly, where the eighteen-tool coverage in <c>RealToolMetaMcpTests</c> catches them, rather
